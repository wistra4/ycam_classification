...
# define directories
base_dir = "./"
esc_dir = os.path.join(base_dir, "master")
meta_file = os.path.join(esc_dir, "meta/ycam.csv")
...
data_dir = esc_dir + "/dataset/"
if not os.path.exists(data_dir):
    os.mkdir(data_dir)
    data_dir = os.path.join(esc_dir, "dataset/")

# save test dataset
if not os.path.exists(data_dir+"esc_melsp_test.npz"):
    save_np_data(data_dir+"esc_melsp_test.npz", x_test,  y_test)

# save raw training dataset
if not os.path.exists(data_dir+"esc_melsp_train_raw.npz"):
    save_np_data(data_dir+"esc_melsp_train_raw.npz", x_train,  y_train)

# save training dataset with white noise
if not os.path.exists(data_dir+"esc_melsp_train_wn.npz"):
    rates = np.random.randint(1,50,len(x_train))/10000
    save_np_data(data_dir+"esc_melsp_train_wn.npz", x_train,  y_train, aug=add_white_noise, rates=rates)

# save training dataset with sound shift
if not os.path.exists(data_dir+"esc_melsp_train_ss.npz"):
    rates = np.random.choice(np.arange(2,6),len(y_train))
    save_np_data(data_dir+"esc_melsp_train_ss.npz", x_train,  y_train, aug=shift_sound, rates=rates)

# save training dataset with stretch
if not os.path.exists(data_dir+"esc_melsp_train_st.npz"):
    rates = np.random.choice(np.arange(80,120),len(y_train))/100
    save_np_data(data_dir+"esc_melsp_train_st.npz", x_train,  y_train, aug=stretch_sound, rates=rates)

# save training dataset with combination of white noise and shift or stretch
if not os.path.exists(data_dir+"esc_melsp_train_com.npz"):
    ...
    np.savez(data_dir+"esc_melsp_train_com.npz", x=np_data, y=np_targets)
...
# dataset files
train_files = [data_dir+"esc_melsp_train_raw.npz", 
               data_dir+"esc_melsp_train_ss.npz",
               data_dir+"esc_melsp_train_st.npz", 
               data_dir+"esc_melsp_train_wn.npz",
               data_dir+"esc_melsp_train_com.npz"]
test_file = data_dir+"esc_melsp_test.npz"
